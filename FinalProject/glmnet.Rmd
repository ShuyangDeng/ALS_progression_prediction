...
# Lasso Regression
set.seed(1234)
lasso <- train(ALSFRS_Slope ~ .-subject_id,data=training_set_clus1, method="glmnet", tuneGrid=expand.grid(alpha=1,lambda=seq(0.0001,0.05,length=5)), trControl=custom )
```
...
# performe almost same, 2 is better than 3
```

```{r}
plot(lasso$finalModel,xvar="dev",label=T)
# 3 is less important than 2
```
...
set.seed(1234)
en <- train(ALSFRS_Slope ~ .-subject_id, training_set_clus1, method="glmnet", tuneGrid=expand.grid(alpha=seq(0,1,length=10), lambda=seq(0.0001, 0.2, length=5)), trControl=custom)
```
...
ggplot(data = cluster_1, mapping = aes(x = ALSFRS_Slope, color = Limb_Onset)) +
  geom_freqpoly(binwidth = 0.1)
```
...
ggplot(data = cluster_2, mapping = aes(x = ALSFRS_Slope, color = Bulbar_Onset)) +
  geom_freqpoly(bins=30)

ggplot(data = cluster_2, mapping = aes(x = ALSFRS_Slope, color =Limb_Onset)) +
  geom_freqpoly(bins=30)
```
