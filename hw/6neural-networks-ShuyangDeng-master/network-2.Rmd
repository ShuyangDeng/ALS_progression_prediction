...
nnetGrid <-  expand.grid(size = seq(from = 2, to = 2, by = 1),
...
```{r, include=FALSE}
X <- data.frame(A = c(0,0,1,1,1,1,1,0), B = c(0,1,1,0,0,0,0,1))
Y <- data.frame(Y = c(0,1,0,1,0,1,0,1))
b <- data.frame(b = c(1,1))



ctrl <- trainControl(method = "repeatedcv", number = 3, repeats = 5, classProbs = TRUE, summaryFunction = LogLosSummary)
...
```
```{r}
prediction_nn <- predict(nnetFit, newdata = X,type="prob")
rocobj <- roc(Y~as.numeric(prediction_nn[[1]]))
...
nnetGrid <-  expand.grid(size = seq(from = 2, to = 3, by = 1),
                        decay = seq(from = 0.1, to = 0.3, by = 0.1))
Y=as.factor(as.matrix(make.names(Y[,1])))
